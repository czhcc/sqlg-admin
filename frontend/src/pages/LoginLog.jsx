      ...
      <header className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-3">
        <div>
          <h1 className="flex items-center gap-2 text-lg font-semibold text-gray-800">
            <LogIn size={20} className="text-indigo-500" /> 登录日志
          </h1>
          <p className="mt-0.5 text-sm text-gray-500">追踪用户登录行为 · 安全审计</p>
        </div>
        ...
        <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {!loading && rows.length === 0 && <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">暂无记录</td></tr>}
              {!loading && rows.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">{formatTime(r.loginTime)}</td>
                  ...
                  <td className="px-4 py-3">
                    {r.resultStatus === 'SUCCESS'
                      ? <span className="inline-flex items-center gap-1 rounded bg-green-50 px-1.5 py-0.5 text-xs text-green-700"><CheckCircle2 size={11} /> 成功</span>
                      : <span className="inline-flex items-center gap-1 rounded bg-red-50 px-1.5 py-0.5 text-xs text-red-700"><XCircle size={11} /> 失败</span>}
                  </td>
